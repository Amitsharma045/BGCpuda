...
public class EmployeeRepository {
	...
	public List<Employee> getEmployee(Role role, Area area) {
		List<Employee> emplist = getSession()
				.createQuery("FROM Employee emp WHERE emp.role.id =:p1 and emp.isActive = 1 and emp.id in (select eam.employee.id FROM EmployeeAreaMapping eam where eam.area.id = :p2)")
